...
export function setupRealtimeListeners(userId, onDataChangeCallback) {
    ...
    dbState = {
        ...
        _realtimeFinanceiro: [], _historyFinanceiro: [],
        _realtimeCustos: [], _historyCustos: [],
        _loadedCollections: new Set() // [NEW] Rastreio de carregamento
    };

    // Coleções leves (Carrega tudo)
    const smallCollections = ['clientes', 'fotografos', 'colunas', 'templates', 'pacotes', 'configuracoes', 'categorias', 'subcategorias', 'vendedores', 'contratos', 'tipos_entrega'];
    ...
    smallCollections.forEach(col => {
        ...
        const unsub = onSnapshot(collection(db, collectionPath), (querySnapshot) => {
            ...
            dbState._loadedCollections.add(col); // [NEW] Marca como carregado
            ...
            if (col === 'subcategorias') dbState.subcategorias.sort((a, b) => a.nome.localeCompare(b.nome)); // [NEW] Sort logic
